...
CW_Parallel_VRP <- function(Sort_Edge, nnodes, routes = list(), Served_nodes = c(), Filled_Routes = c(), logfile = TRUE, Constraints = c("Capacity")){
  ...
  if(nCommonNodes == 0){ # If there is no common node between new edge and existing routes then create a new route
    ...
    return(CW_Parallel_VRP(Sort_Edge[-1, ], nnodes, routes = routes, Served_nodes = Served_nodes, Constraints = Constraints))
