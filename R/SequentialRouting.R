...
CW_Sequential_VRP <- function(Sort_Edge, nnodes, routes = list(), Served_nodes = c(), Filled_Routes = c(), logfile = TRUE, Constraints = c("Capacity")){
  ...
  if(nCommonNodes == 0){ # If there is no common node between new edge and existing routes then create a new route
    ...
    if(x == 0){
      ...
      return(CW_Sequential_VRP(Sort_Edge[-1, ], nnodes, routes = routes, Served_nodes = Served_nodes, Constraints = Constraints))
    }else{
      ...
      return(CW_Sequential_VRP(Sort_Edge, nnodes, routes = routes, Served_nodes = Served_nodes, Constraints = Constraints))
    }
    ...
    CC <- CheckConstraints(routes[[route_id]], CommonNode, NewNode, Constraints)
